...
void Browser_setup() {
  ...
  //   MySerial.printf("STA: Failed!\n");
  //   WiFi.disconnect(false);
  //   delay(500);
  //   WiFi.begin(ssid, password);
  // }
  MySerial.println("IP Address: " + WiFi.localIP().toString());
  ...
  if (!StartMDNSservice(ServerName)) {
    MySerial.println("Error starting mDNS Service...");;
    StartupErrors = true;
  }
  if (!FSys.begin(true)) {
    MySerial.println("Error preparing Filing System...");
    StartupErrors = true;
  }

  // ##################### HOMEPAGE HANDLER ###########################
  server1.on("/file", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### LOGOUT HANDLER ############################
  server1.on("/logout", HTTP_GET, [](AsyncWebServerRequest * request) {
    LogOut();
    request->send(200, "text/html", webpage);
  });

  // ##################### DOWNLOAD HANDLER ##########################
  server1.on("/download", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### UPLOAD HANDLERS ###########################
  server1.on("/upload", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  server1.on("/handleupload", HTTP_POST, [](AsyncWebServerRequest * request) {},
  ...
  server1.on("/handleformat", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### RENAME HANDLER ############################
  server1.on("/rename", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### DIR HANDLER ###############################
  server1.on("/dir", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### DELETE HANDLER ############################
  server1.on("/delete", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### FORMAT HANDLER ############################
  server1.on("/format", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### SYSTEM HANDLER ############################
  server1.on("/system", HTTP_GET, [](AsyncWebServerRequest * request) {
    Display_System_Info(); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });

  // ##################### IMAGE HANDLER ############################
  server1.on("/icon", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### NEW PAGE HANDLER ##########################
  server1.on("/newpage", HTTP_GET, [](AsyncWebServerRequest * request) {
    Display_New_Page(); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });

  server1.begin();  // Start the server
  if (!StartupErrors)
    MySerial.println("System started successfully...");
  else
    MySerial.println("There were problems starting all services...");
