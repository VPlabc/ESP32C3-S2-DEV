...
//########################################################################################pragma endregion
void saveMemoryToFile(int saved, bool storeFound, uint8_t * sensors, size_t dataSize) {

//LOG ("saveMemoryToFile: " + String(SPIFFSFunction::SPIFFS_card_found) +"\n");
  if (storeFound == true) {
    if (SPIFFS.exists("/memory.bin")) {SPIFFS.remove("/memory.bin");}
    File file = SPIFFS.open("/memory.bin", FILE_WRITE);
    if (file) {
      file.write(saved);
       for (int i = 0; i < saved; i++) file.write((uint8_t *)&sensors[i], dataSize);
      file.close();
    }
  }
}

void readMemoryFromFile(int saved, bool storeFound , uint8_t * sensors, size_t dataSize) {
//LOG ("readMemoryFromFile: " + String(storeFound) + "\n");
    // uint8_t *sensors[dataSize];
  if (storeFound == true) {
    if (SPIFFS.exists("/memory.bin")) {
      File file = SPIFFS.open("/memory.bin", FILE_READ);
      saved = file.read();//LOG (String(sensors_saved) + " Device Exists\n");
      for (int i = 0; i <saved; i++) file.read((uint8_t *)&sensors[i], dataSize);
      file.close();
    }
    else{
    LOG ("File not Exists\n");
